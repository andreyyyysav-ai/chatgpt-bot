...
# === КОНФИГУРАЦИЯ ===
MODEL = "openai/gpt-oss-20b"          # ✅ СТАБИЛЬНАЯ РАБОЧАЯ МОДЕЛЬ
FREE_WAIT = 10
IMAGE_WAIT = 15
MAX_CONTEXT = 50
MAX_MEMORY = 50
...
# === СУПЕР-ОЧИСТКА ОТВЕТА (агрессивная) ===
def clean_response(text: str) -> str:
    # 1. Удаляем все теги и маркеры рассуждений
    patterns = [
        r'<think>.*?</think>',
        r'<thinking>.*?</thinking>',
        r'<thought>.*?</thought>',
    ...
    # 2. Удаляем любые английские буквы, цифры, точки, запятые,
    #    которые образуют "мусор" типа ss1.ssss, 2.s и т.п.
    #    Оставляем только русские буквы, пробелы, знаки препинания (!?.,)
    #    и цифры (но только если они часть слова, например, "27", "100")
    #    Однако часто мусор — это последовательности из английских букв и цифр с точками.
    #    Лучше удалить всё, что не является русским текстом, пробелами и базовой пунктуацией.
    #    Но при этом сохранить эмодзи? Эмодзи не удаляем.
    #    Для простоты: удаляем любые последовательности, где есть английские буквы.
    #    Сначала разбиваем на слова, проверяем каждое слово: если в нём есть латиница,
    #    и оно не является общеизвестным (например, "OK", "GPT"), то удаляем.
    #    Но проще: оставить только кириллицу, пробелы, цифры и знаки . , ! ? -
    #    Также точка может быть внутри числа (например, 3.14) — оставим.
    #    Реализуем через re.sub с разрешёнными символами.
    
    # Разрешаем: русские буквы, пробелы, цифры, . , ! ? - и эмодзи (но эмодзи мы не трогаем)
    # Эмодзи не попадают в этот набор, поэтому их не удалим.
    # Удалим всё, что не входит в: А-Яа-яЁё 0-9 .,!?- и пробел
    # Но это удалит и эмодзи. Чтобы сохранить эмодзи, нужно их распознать.
    # Проще: удалим только явный мусор — английские буквы, которые идут подряд с точками и цифрами.
    # Заменим всё, что похоже на "ss1.ssss" или "2.s" на пустоту.
    text = re.sub(r'\b[a-zA-Z]+\d*\.?[a-zA-Z]*\d*\b', '', text)  # удаляем слова с латиницей
    text = re.sub(r'\d+\.\s*[a-zA-Z]', '', text)  # удаляем цифры с точкой и латиницу
    text = re.sub(r'[a-zA-Z]{2,}', '', text)  # удаляем любые последовательности латиницы длиннее 1 (чтобы не задеть "OK")
    # Но "OK" тоже удалится, но это не страшно.
    
    # 3. Удаляем лишние точки, запятые, пробелы
    text = re.sub(r'\.{2,}', '.', text)  # множественные точки в одну
    text = re.sub(r',{2,}', ',', text)
    text = re.sub(r'\s{2,}', ' ', text)
    text = re.sub(r'\n\s*\n', '\n\n', text)
    text = text.strip()
    
    # 4. Если после очистки остались только цифры или знаки — возвращаем заглушку
    if not text or len(text) < 2 or not re.search(r'[А-Яа-я]', text):
        return "Привет! Я ChatGPT, твой бесплатный ассистент. Чем могу помочь? 😊🚀"
    
    # 5. Убеждаемся, что текст начинается с большой буквы
